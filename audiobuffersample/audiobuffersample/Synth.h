...
typedef enum
{
	...
	STATE_RELEASED,      ///< ToneEvent is released and ringing out
}
ToneEventState;
...
@interface Synth : NSObject
{
	...
	float* sine;        ///< look-up table for quick \c sin()
	int sineLength;     ///< size of sine look-up table
	
	float* envelope;    ///< look-up table for the envelope
	int envLength;      ///< size of envelope look-up table
    
	/// the slots for the tones
